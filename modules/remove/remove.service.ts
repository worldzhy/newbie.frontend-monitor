...
export class RemoveService {
  ...
  async customDelete(fil: any) {
    ...
    const startTime = moment(new Date(time[0]).valueOf()).format(
      'YYYY-MM-DD 00:00:00',
    );
    const endTime = moment(new Date(time[1]).valueOf()).format(
      'YYYY-MM-DD 23:59:59',
    );
      ...
      if (resource && resource.length > 0) {
        ...
        for (const item of resource) {
          switch (item) {
            ...
            case 'page': {
              arr.push(
                Promise.resolve(this.mongo.WxPage(appId).deleteMany(query).exec()),
              );
              break;
            }
            case 'err': {
      ...
      if (resource && resource.length > 0) {
        ...
        for (const item of resource) {
          ...
          switch (item) {
            ...
            case 'page': {
              arr.push(
                Promise.resolve(this.mongo.WebPage(appId).deleteMany(query).exec()),
              );
              break;
            }
            case 'env': {
              arr.push(
                Promise.resolve(
                  this.mongo.WebEnvironment(appId).deleteMany(query).exec(),
                ),
              );
              break;
            }
            ...
            case 'resource': {
              arr.push(
                Promise.resolve(
                  this.mongo.WebResource(appId).deleteMany(query).exec(),
                ),
              );
            }
          }
